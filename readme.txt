A MATLAB MEX wrapper for the International Reference Ionosphere Model

A MEX file allows users to execute FORTRAN code from within MATLAB.
This repository adds three MATLAB functions, one to interface with the MEX code, 
the other two to automatically download the apf107.dat and ig_rz.dat files to provide 
needed indices to the IRI MSIS. 

IRI2020.m should automatically compile a MEX file when first run.

By default, the IRI will be configured as the default specific in IRISUB.F.

See IRI2020.m for details.	
		
NOTE: The Fortran code in IRIFUN.F, IGRF.F, CIRA.F have been modified to remove
instances of the STOP statement. This statement tends to produce unpredictable 
behaviour in MATLAB when called in a MEX file, up to and including killing all 
active MATLAB instances. Care should be taken when updating the IRI code.
